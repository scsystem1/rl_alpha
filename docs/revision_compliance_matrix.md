...
| Unified compact 2B prompt | `search/prompts.py`, `benchmark_prompts.py` | implemented; fresh GPU evidence pending | one `unified_compact_v7` contract shared by Base-LLM/GRPO; finite featureful depth-six grammar, full pool, elements, concise goal and combined hints; no version/hint switch | Re-profile tokens and run fresh one-prompt/eight-answer GPU smoke | prompt contract/sampling tests |
