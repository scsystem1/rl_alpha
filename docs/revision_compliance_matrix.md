...
| Pool reward has one complete sample and finite failure semantics | `rewards/base.py`, `factors/pool.py` | verified | no missing-as-zero moments; joint signal/label projection; NaN/Inf gets penalty and reason | None | hand recomputation/zero-variance tests |
...
| One serialized fit/OOS transform | `factors/transform.py`, `RidgeCombiner` | verified | daily 1/99 winsorization, z-score, joint projection, post-z-score once; zero-variance days removed; round trip identical | None | synthetic hand/serialization + real CPU smoke |
| Per-cell complete-case evaluation support | `evaluation/finalize.py` | unit verified | requested method cells are independent; valid days/observations recorded without cross-method hard gate | real multi-method comparison pending | method-filter unit + smoke artifacts |
| RNIC uses identical signal/label projection sample | transform/finalize | verified | daily joint matrix on `trade ∩ finite(signals,label,exposures)` | None | unequal-missingness orthogonality test |
...
| Unified compact 2B prompt | `search/prompts.py`, `benchmark_prompts.py` | implemented; fresh GPU evidence pending | one `unified_compact_v1` contract shared by Base-LLM/GRPO; full pool, elements, concise goal and combined hints; no version/hint switch | Re-profile tokens and run fresh one-prompt/eight-answer GPU smoke | prompt contract/sampling tests |
