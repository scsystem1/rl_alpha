...
def _pool_lines(context: SearchContext) -> str:
    return "\n".join(POOL_ROW_TEMPLATE.format(formula=formula) for formula in context.pool_formulas) or "- empty"
...
def build_messages(context: SearchContext) -> list[dict[str, str]]:
    ...
    user = USER_TEMPLATE.format(pool=_pool_lines(context),
...
def prompt_contract() -> dict[str, object]:
    payload = {
        ...
        "summary": "none; original v7 formula-only pool and diversity hints",
        "grammar": DSL_GRAMMAR,
